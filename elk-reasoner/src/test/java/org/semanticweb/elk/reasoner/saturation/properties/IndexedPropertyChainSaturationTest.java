...
public class IndexedPropertyChainSaturationTest {
	...
	public void testCyclicCompositions() {
		...
		//now make R composable with itself
		IndexedPropertyChain RR = IndexedObjectsCreator.createIndexedChain(R, R, new IndexedObjectProperty[]{H});
		...
		assertTrue(R.getSaturated().getCompositionsByLeftSubProperty().get(R).contains(H));
		assertTrue(R.getSaturated().getCompositionsByRightSubProperty().get(R).contains(H));
	}
	...
	public void testReflexivity() {
		...
		toldReflexive.addAll(Arrays.asList(new IndexedObjectProperty[]{r1, h}));
		
		final List<IndexedPropertyChain> chains = Arrays.asList(new IndexedPropertyChain[]{t, r1, r, r1r1, rt, u, h, h1, s, rh1, v1, v2, v1v1, v2v2});
		final Set<IndexedPropertyChain> correctReflexive = new HashSet<IndexedPropertyChain>(Arrays.asList(new IndexedPropertyChain[]{r, h, h1, r1, r1r1, rh1, s}));
		//create the factory and run the computation
		int maxThreads = Runtime.getRuntime().availableProcessors(); 
		...
		for (IndexedPropertyChain chain : chains) {
			assertEquals(chain.toString(), correctReflexive.contains(chain), isReflexive(chain));
		}
	}
