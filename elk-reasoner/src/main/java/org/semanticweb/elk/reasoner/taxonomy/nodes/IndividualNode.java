...
public class IndividualNode implements
		InstanceNode<ElkIri, ElkClass, ElkIri, ElkNamedIndividual> {
	 ...
	 * node.
	 */
	private final Set<UpdateableBottomTypeNode<ElkIri, ElkClass, ElkIri, ElkNamedIndividual>> directTypeNodes_;
	/**
	...
	protected IndividualNode(Collection<ElkNamedIndividual> members) {
		...
		this.directTypeNodes_ = new ArrayHashSet<UpdateableBottomTypeNode<ElkClass, ElkNamedIndividual>>();
	...
	@Override
	public Set<UpdateableBottomTypeNode<ElkClass, ElkNamedIndividual>> getDirectTypeNodes() {
		return Collections.unmodifiableSet(directTypeNodes_);
	}

	@Override
	public Set<UpdateableBottomTypeNode<ElkClass, ElkNamedIndividual>> getAllTypeNodes() {
		Set<UpdateableBottomTypeNode<ElkClass, ElkNamedIndividual>> result = new ArrayHashSet<UpdateableBottomTypeNode<ElkClass, ElkNamedIndividual>>(
				directTypeNodes_.size());

		Queue<UpdateableBottomTypeNode<ElkClass, ElkNamedIndividual>> todo = new LinkedList<UpdateableBottomTypeNode<ElkClass, ElkNamedIndividual>>();

		todo.addAll(directTypeNodes_);

		while (!todo.isEmpty()) {
			UpdateableBottomTypeNode<ElkClass, ElkNamedIndividual> next = todo.poll();

			if (result.add(next)) {
				for (UpdateableBottomTypeNode<ElkClass, ElkNamedIndividual> nextSuperNode : next
