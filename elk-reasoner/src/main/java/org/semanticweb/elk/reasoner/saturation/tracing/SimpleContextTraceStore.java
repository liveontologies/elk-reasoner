...
public class SimpleContextTraceStore implements ContextTraceStore {
	...
	private final ConclusionVisitor<InferenceVisitor<?,?>, Void> inferenceReader_ = new AbstractConclusionVisitor<InferenceVisitor<?,?>, Void>() {
		...
		private void visitBackwardLinkInferences(IndexedPropertyChain relation, IndexedClassExpression source, InferenceVisitor<?, ?> visitor) {
			synchronized (relation) {
				Multimap<IndexedClassExpression, Inference> sourceToInferenceMap = backwardLinkInferenceMap_.get(relation);
		...
		@Override
		public Void defaultVisit(Conclusion conclusion, InferenceVisitor<?, ?> input) {
			return null;
		}
		
	}; 
	
	private final InferenceVisitor<?, Boolean> inferenceWriter_ = new BaseInferenceVisitor<Void, Boolean>() {
	...
	protected Boolean addTracedBackwardLink(IndexedPropertyChain relation, IndexedClassExpression source, Inference link) {
	...
	public void visitInferences(InferenceVisitor<?, ?> visitor) {
		...
		for (IndexedPropertyChain linkRelation : backwardLinkInferenceMap_
				.keySet()) {
			Multimap<IndexedClassExpression, Inference> contextMap = backwardLinkInferenceMap_.get(linkRelation); 
		...
		for (IndexedPropertyChain linkRelation : forwardLinkInferenceMap_.keySet()) {
			...
			Multimap<IndexedClassExpression, Inference> contextMap = forwardLinkInferenceMap_.get(linkRelation);
