...
public class ReasonerConfiguration {
	...
	@Parameter(type="org.semanticweb.elk.reasoner.config.NumberOfWorkers")//the only reason we don't use java.lang.Integer here is because the default value is not a constant
	...
	static {		
		...
		for (Field field : allFields) {
			if (field.isAnnotationPresent(Parameter.class)) {
				...
				try {
					...
					throw new ReasonerConfigurationException("Perhaps incorrect declaration of the configuration parameter " + field.getName(), e);
				}
		...
		defaultConfig = new ReasonerConfiguration(Collections.unmodifiableMap(defaultMap));
	}
	...
	public ReasonerConfiguration clone() {
		return new ReasonerConfiguration(new HashMap<String, String>(this.paramMap));
	}
	...
	public void setParameter(String name, String value) {
		if (!validate(name, value)) throw new ReasonerConfigurationException("Wrong value " + value + " for parameter " + name);
	...
	private boolean validate(String name, String value) {
		...
		try {
			if (validator != null) validator.create(value);
		} catch (Exception e) {
			return false;
		}
...
/**
 * Validates the string-based representation of each parameter by trying to instantiate it
 * 
 * @author Pavel Klinov
 *
 * pavel.klinov@uni-ulm.de
