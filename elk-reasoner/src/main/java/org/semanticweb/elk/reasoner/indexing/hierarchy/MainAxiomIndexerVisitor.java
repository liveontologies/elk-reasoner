...
public class MainAxiomIndexerVisitor extends AbstractElkAxiomIndexerVisitor
	 ...
	 */
	public MainAxiomIndexerVisitor(ModifiableOntologyIndex index, ElkDatatypeHandler datatypeHandler, boolean insert) {
		this.index_ = index;
		...
		this.neutralIndexer = new IndexObjectConverter(
				...
				propertyOccurrenceUpdateFilter, datatypeHandler);
		...
		this.positiveIndexer = new IndexObjectConverter(
				...
				propertyOccurrenceUpdateFilter, negativeIndexerFactory, datatypeHandler);
	...
	public void indexSubObjectPropertyOfAxiom(
		...
		if (multiplicity_ == 1) {
			subIndexedProperty.addToldSuperProperty(superIndexedProperty);
			superIndexedProperty.addToldSubPropertyChain(subIndexedProperty);
		} else {
			subIndexedProperty
					.removeToldSuperProperty(superIndexedProperty);
			superIndexedProperty
					.removeToldSubProperty(subIndexedProperty);
		}
	}
	...
	private class PropertyOccurrenceUpdateFilter implements
		...
		public <T extends IndexedDataProperty> T update(T idp) {
			if (!idp.occurs() && increment > 0)
				index_.add(idp);

			idp.updateOccurrenceNumber(increment);
