...
/**
 * The engine for setting up the compositions multimap in
 * {@link SaturatedPropertyChain} that are used in {@link RuleRoleComposition}.
 * 
 * @author Frantisek Simancik
 */
class ObjectPropertyCompositionsPrecomputationEngine implements
		InputProcessor<IndexedPropertyChain> {

	/**
	 * If set to true, then binary property chains that do not occur negatively
	 * are skipped in the derivation and replaced directly by all their told
	 * superproperties. For example, given an inclusion
	 * SubObjectPropertyOf(ObjectPropertyChain(R1 R2) R), the composition of R1
	 * and R2 derives directly R skipping the auxiliary binary chain
	 * representing [R1 R2].
	 */
	static final boolean REPLACE_CHAINS_BY_TOLD_SUPER_PROPERTIES = true;
	 ...
	 */
	static final boolean ELIMINATE_IMPLIED_COMPOSITIONS = true;
	...
	public void submit(IndexedPropertyChain rightProperty) {
		...
		// rightProperty on the right and register them for the rightProperty
	...
	/**
	 * Add (S, chain) to rightPropertySaturated.compositionsByLeftSubProperty
	 * for each subproperty S of chain.leftProperty. The compositions are not
	 * yet registered for the left components.
	 */
	void registerComposition(IndexedBinaryPropertyChain chain,
		...
		for (IndexedPropertyChain leftProperty : chain.getLeftProperty()
			...
			/*
			 * If the chain does not occur negatively, then replace it by its
	...
	/**
	 * An implementation of the {@link AbstractHashMultimap} that uses Vectors
