...
public class PlainLiteralDatatypeHandler implements DatatypeHandler {
	...
	private ValueSpace createLiteralValueSpace(IndexedDataHasValue datatypeExpression) {
		...
		switch (datatype) {
			...
			case xsd_string:
				effectiveDatatype = xsd_string;
			case xsd_normalizedString:
				if (isValidNormalizedString(value))
				effectiveDatatype = xsd_normalizedString;
			case xsd_token:
				if (isValidToken(value))
				effectiveDatatype = xsd_token;
			case xsd_NMTOCKEN:
				if (isValidNMToken(value))
				effectiveDatatype = xsd_NMTOCKEN;
			case xsd_Name:
				if (isValidName(value))
				effectiveDatatype = xsd_Name;
			case xsd_NCName:
				if (isValidNCName(value))
				effectiveDatatype = xsd_NCName;
		}
	 ...
	 * characters.
	 *
	 * @param nstring string to check
	 * @return true if name is a valid normalizedString
	 */
	private boolean isValidNormalizedString(String nstring) {
		int scan;

		for (scan = 0; scan < nstring.length(); scan++) {
			char cDigit = nstring.charAt(scan);
			switch (cDigit) {
				case 0x09:
				case 0x0A:
				case 0x0D:
	 ...
	 * @return true if name is a valid token
	 */
	public static boolean isValidToken(String token) {
		...
		// no leading space
		if (token.charAt(0) == 0x20) {
			return false;
		}

		// no trail space
		if (token.charAt(token.length() - 1) == 0x20) {
			return false;
		}

		for (scan = 0; scan < token.length(); scan++) {
			char cDigit = token.charAt(scan);
			switch (cDigit) {
				case 0x09:
				case 0x0A:
					return false;
				case 0x20:
					// no doublspace
					if (scan + 1 < token.length()) {
						if (token.charAt(scan + 1) == 0x20) {
							return false;
						}
					}
				default:
