...
class SaturatedObjectPropertyImpl implements SaturatedObjectProperty {
	...
	public SaturatedObjectPropertyImpl(IndexedObjectProperty iop) {
		compute(iop);
	}
	...
	protected void compute(IndexedObjectProperty iop) {		
		...
		derivedSubObjectProperties.add(iop);
		queue.addLast(iop);
		while (!queue.isEmpty()) {
		...
		queue.clear();
		derivedSuperObjectProperties.add(iop);
		queue.addLast(iop);
		while (!queue.isEmpty()) {
			IndexedObjectProperty r = queue.removeLast();
