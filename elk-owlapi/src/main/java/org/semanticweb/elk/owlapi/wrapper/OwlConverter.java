...
import org.semanticweb.elk.owlapi.ConverterException;
...
public class OwlConverter {
	...
	private static Owl2LiteralParser literalParser_ = new Owl2LiteralParser();
	...
	public ElkLiteral convert(OWLLiteral owlLiteral) {
		...
		if (owlDatatype != null && (owlLiteral.getLang() == null || owlLiteral.getLang().isEmpty())) {
			...
			try {
				return literalParser_.createLiteral(owlLiteral.getLiteral(), datatype);
			} catch (Owl2ParseException e) {
				throw new ConverterException("Datatype parsing failed", e);
			}
		}
		else {
